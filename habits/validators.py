...
class HabitValidator:
    ...
    def __call__(self, data):
        ...
        if duration.total_seconds() > 120:
            raise ValidationError("Время выполнения привычки не должно привышать 120 секунд.")
        ...
        if frequency < 1 or frequency > 7:
            raise ValidationError("Периодичность выполнения должна быть от 1 до 7 дней (не реже одного раза в неделю).")
